...
function getToastIcon(type: ToastProps["type"], stage?: string) {
  switch (type) {
    ...
    case "version-update":
      return (
        <LuRefreshCw className="size-4 shrink-0 animate-spin text-foreground motion-reduce:animate-none" />
      );
    case "fetching":
      return (
        <LuRefreshCw className="size-4 shrink-0 animate-spin text-foreground motion-reduce:animate-none" />
    ...
    case "loading":
      return (
        <div className="size-4 shrink-0 animate-spin rounded-full border-2 border-foreground border-t-transparent motion-reduce:animate-none" />
      );
    default:
      return (
        <div className="size-4 shrink-0 animate-spin rounded-full border-2 border-foreground border-t-transparent motion-reduce:animate-none" />
...
export function UnifiedToast(props: ToastProps) {
  ...
  return (
    <div className="flex w-full max-w-md items-start rounded-lg border border-border bg-card p-3 text-card-foreground shadow-lg">
      <div className="mt-0.5 mr-3" aria-hidden="true">
        {getToastIcon(type, stage)}
      </div>
      <div className="min-w-0 flex-1" data-slot="toast-content">
            ...
            <div className="mt-2 space-y-1">
              ...
              progress.percentage > 0 ? (
                ...
                <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
                  <div className="h-1.5 w-1/3 animate-progress-indeterminate rounded-full bg-foreground motion-reduce:animate-none" />
                </div>
              )}
            </div>
          )}
